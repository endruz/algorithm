...
states: List[State] = [
    State(row=0, col=0, direction="EAST", step_length=10, heat_loss=0),
    State(row=0, col=0, direction="SOUTH", step_length=10, heat_loss=0),
]
...
while states:
    ...
    if state.row == height - 1 and state.col == weight - 1 and state.step_length >= 4:
        result = state.heat_loss
        break

    # maybe it could look prettier?
    if state.direction == "EAST":
        if state.step_length >= 4:
        ...
        if state.step_length < 10 and (col := state.col + 1) < weight:
        ...
        if state.step_length >= 4:
            ...
            if (row := state.row - 1) >= 0:
                heapq.heappush(states, State(row=row, col=state.col, direction="NORTH", step_length=1, heat_loss=state.heat_loss+grid[row][state.col]))
        if state.step_length < 10 and (col := state.col - 1) >= 0:
        ...
        if state.step_length < 10 and (row := state.row + 1) < height:
        ...
        if state.step_length >= 4:
            ...
            if (col := state.col - 1) >= 0:
                heapq.heappush(states, State(row=state.row, col=col, direction="WEST", step_length=1, heat_loss=state.heat_loss+grid[state.row][col]))
        if state.step_length < 10 and (row := state.row - 1) >= 0:
